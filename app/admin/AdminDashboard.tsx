...
export default function AdminDashboard() {
  ...
  const savePackage = async () => {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        setError(e.error || "Unable to save.");
      } else {
  ...
  const deletePackage = async () => {
    ...
    setMessage(null);
    try {
      const r = await fetch(`/api/admin/packages/${selectedPackageId}`, {
        method: "DELETE",
      });
      if (!r.ok) {
        const e = await r.json();
        setError(e.error || "Unable to delete.");
      } else {
  ...
  const uploadImage = async (file: File, field: "coverImage" | "gallery") => {
    ...
    fd.append("file", file);
    try {
      const r = await fetch("/api/admin/packages/upload", {
        method: "POST",
        body: fd,
      });
      if (!r.ok) {
  ...
  const uploadPdf = async (file: File) => {
    ...
    fd.append("file", file);
    try {
      const r = await fetch("/api/admin/packages/upload-pdf", {
        method: "POST",
        body: fd,
      });
      if (!r.ok) {
              ...
              <main className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                {!pkg ? (
                  ...
                  <>
                    ...
                    <div className="space-y-4">
                        ...
                        <>
                          <label className="block space-y-2 text-sm">
                            ...
                            <input
                              value={pkg.id}
                              onChange={(e) =>
                                updatePkg({ id: e.target.value })
                              }
                              className="w-full rounded-2xl border px-4 py-3"
                              placeholder="capital-crown-kashmir"
                            />
                          </label>
                          <label className="block space-y-2 text-sm">
                            <span>Title</span>
                            <input
                              value={pkg.title}
                              onChange={(e) =>
                                updatePkg({ title: e.target.value })
                              }
                              className="w-full rounded-2xl border px-4 py-3"
                              placeholder="Capital, Crown & Kashmir"
                            />
                          ...
                          <label className="block space-y-2 text-sm">
                            ...
                            <div className="flex items-center rounded-2xl border overflow-hidden">
                              <input
                                ...
                                value={pkg.duration}
                                onChange={(e) =>
                                  updatePkg({ duration: e.target.value })
                                }
                                className="flex-1 px-4 py-3 outline-none"
                                placeholder="7"
                              />
                              <span className="px-4 py-3 bg-slate-50 text-slate-500 text-sm border-l select-none">
                                Days
                              </span>
                            </div>
                          </label>
                          <label className="block space-y-2 text-sm">
                            <span>Price</span>
                            <div className="flex items-center rounded-2xl border overflow-hidden">
                              <span className="px-4 py-3 bg-slate-50 text-slate-500 text-sm border-r select-none">
                                ₹
                              </span>
                              <input
                                type="number"
                                ...
                                value={pkg.price}
                                onChange={(e) =>
                                  updatePkg({ price: e.target.value })
                                }
                                className="flex-1 px-4 py-3 outline-none"
                                placeholder="39999"
                              />
                          ...
                          <label className="block space-y-2 text-sm">
                            ...
                            <div className="space-y-2">
                              <input
                                value={pkg.coverImage}
                                onChange={(e) =>
                                  updatePkg({ coverImage: e.target.value })
                                }
                                className="w-full rounded-2xl border px-4 py-3"
                                placeholder="/packages/images/example.jpg"
                              />
                              <input
                                type="file"
                                accept="image/*"
                          ...
                          <label className="block space-y-2 text-sm">
                            ...
                            <textarea
                              ...
                              onChange={(e) =>
                                updatePkg({ shortDescription: e.target.value })
                              }
                              className="w-full rounded-2xl border px-4 py-3 min-h-20"
                            />
                          </label>
                          <label className="block space-y-2 text-sm">
                            <span>Overview</span>
                            <textarea
                              value={pkg.overview}
                              onChange={(e) =>
                                updatePkg({ overview: e.target.value })
                              }
                          ...
                          <div className="flex items-center justify-between rounded-2xl border px-4 py-3">
                            <div>
                              ...
                              <p className="text-xs text-slate-500 mt-0.5">
                                Only published packages are visible to visitors
                              </p>
                            </div>
                            <Toggle
                              value={!!pkg.published}
                              onChange={(v) => updatePkg({ published: v })}
                            />
                          </div>
                          <div className="flex items-center justify-between rounded-2xl border px-4 py-3">
                            <div>
                              ...
                              <p className="text-xs text-slate-500 mt-0.5">
                                Highlighted packages appear on the homepage
                              </p>
                            </div>
                            <Toggle
                              value={!!pkg.highlighted}
                              onChange={(v) => updatePkg({ highlighted: v })}
                            />
                          </div>
                        </>
                      )}
                      ...
                      {activeTab === "destinations" && (
                        <div className="space-y-3">
                            ...
                            {pkg.destination.map((d, i) => (
                              <div key={i} className="flex gap-2">
                                <input
                                  value={d}
                                  onChange={(e) => {
                                    const u = [...pkg.destination];
                                    u[i] = e.target.value;
                                    updatePkg({ destination: u });
                                  }}
                                ...
                                <button
                                  onClick={() =>
                                    updatePkg({
                                      destination: pkg.destination.filter(
                                        (_, j) => j !== i,
                                      ),
                                    })
                                  }
                          ...
                          <button
                            ...
                            className="flex items-center gap-2 rounded-full border border-slate-300 px-4 py-2 text-sm font-medium hover:bg-slate-50"
                          >
                      ...
                      {activeTab === "highlights" && (
                        <div className="space-y-3">
                            ...
                            {pkg.highlights.map((h, i) => (
                              <div key={i} className="flex gap-2">
                                <input
                                  value={h}
                                  onChange={(e) => {
                                    const u = [...pkg.highlights];
                                    u[i] = e.target.value;
                                    updatePkg({ highlights: u });
                                  }}
                                  className="flex-1 rounded-2xl border px-4 py-3 text-sm"
                                />
                                <button
                                  onClick={() =>
                                    updatePkg({
                                      highlights: pkg.highlights.filter(
                                        (_, j) => j !== i,
                                      ),
                                    })
                                  }
                          ...
                          <button
                            onClick={() =>
                              updatePkg({ highlights: [...pkg.highlights, ""] })
                            }
                            className="flex items-center gap-2 rounded-full border border-slate-300 px-4 py-2 text-sm font-medium hover:bg-slate-50"
                          >
                            {/* <Plus className="w-4 h-4" />  */}
                      ...
                      {activeTab === "inclusions" && (
                        <div className="space-y-3">
                            ...
                            {pkg.inclusions.map((inc, i) => (
                              <div key={i} className="flex gap-2">
                                <input
                                  value={inc}
                                  onChange={(e) => {
                                    const u = [...pkg.inclusions];
                                    u[i] = e.target.value;
                                    updatePkg({ inclusions: u });
                                  }}
                                  className="flex-1 rounded-2xl border px-4 py-3 text-sm"
                                />
                                <button
                                  onClick={() =>
                                    updatePkg({
                                      inclusions: pkg.inclusions.filter(
                                        (_, j) => j !== i,
                                      ),
                                    })
                                  }
                          ...
                          <button
                            onClick={() =>
                              updatePkg({ inclusions: [...pkg.inclusions, ""] })
                            }
                            className="flex items-center gap-2 rounded-full border border-slate-300 px-4 py-2 text-sm font-medium hover:bg-slate-50"
                          >
                            {/* <Plus className="w-4 h-4" />  */}
                      ...
                      {activeTab === "exclusions" && (
                        <div className="space-y-3">
                            ...
                            {pkg.exclusions.map((exc, i) => (
                              <div key={i} className="flex gap-2">
                                <input
                                  value={exc}
                                  onChange={(e) => {
                                    const u = [...pkg.exclusions];
                                    u[i] = e.target.value;
                                    updatePkg({ exclusions: u });
                                  }}
                                  className="flex-1 rounded-2xl border px-4 py-3 text-sm"
                                />
                                <button
                                  onClick={() =>
                                    updatePkg({
                                      exclusions: pkg.exclusions.filter(
                                        (_, j) => j !== i,
                                      ),
                                    })
                                  }
                          ...
                          <button
                            onClick={() =>
                              updatePkg({ exclusions: [...pkg.exclusions, ""] })
                            }
                            className="flex items-center gap-2 rounded-full border border-slate-300 px-4 py-2 text-sm font-medium hover:bg-slate-50"
                          >
                            {/* <Plus className="w-4 h-4" />  */}
                              ...
                              >
                                <div className="flex items-center justify-between">
                                  ...
                                  <button
                                    onClick={() =>
                                      updatePkg({
                                        itinerary: pkg.itinerary.filter(
                                          (_, j) => j !== i,
                                        ),
                                      })
                                    }
                                ...
                                <input
                                  value={day.title}
                                  onChange={(e) => {
                                    const u = [...pkg.itinerary];
                                    u[i] = { ...u[i], title: e.target.value };
                                    updatePkg({ itinerary: u });
                                  }}
                                  placeholder="Title"
                                ...
                                <textarea
                                  ...
                                  className="w-full rounded-2xl border px-4 py-3 text-sm min-h-20"
                                />
                                <input
                                  value={day.meals || ""}
                                  onChange={(e) => {
                                    const u = [...pkg.itinerary];
                                    u[i] = { ...u[i], meals: e.target.value };
                                    updatePkg({ itinerary: u });
                                  }}
                        ...
                        <div className="space-y-4">
                          {(
                            ...
                            <div key={key}>
                              ...
                              <div className="space-y-2">
                                {(pkg.termsAndConditions?.[key] || []).map(
                                  (p, i) => (
                                    <div key={i} className="flex gap-2">
                                      <textarea
                                        value={p}
                                        ...
                                        className="flex-1 rounded-2xl border px-4 py-3 text-sm min-h-16"
                                      />
                                      <button
                                        onClick={() => {
                                          const u = (
                              ...
                              <button
                                ...
                                className="mt-2 flex items-center gap-2 rounded-full border border-slate-300 px-3 py-2 text-sm font-medium hover:bg-slate-50"
                              >
                                {/* <Plus className="w-4 h-4" />  */}
                                Add policy
                              </button>
                          ...
                          <div className="border-t border-slate-200 pt-4">
                            <label className="block space-y-2 text-sm">
                              ...
                              <textarea
                                value={pkg.termsNote || ""}
                                onChange={(e) =>
                                  updatePkg({ termsNote: e.target.value })
                                }
                                className="w-full rounded-2xl border px-4 py-3 min-h-24"
                              />
                            </label>
                          </div>
                        </div>
                    ...
                    {message && (
                      <div
                        className={`mt-6 rounded-2xl px-4 py-3 text-sm font-medium ${messageType === "success" ? "bg-emerald-50 text-emerald-800 border border-emerald-200" : "bg-red-50 text-red-800 border border-red-200"}`}
                      >
                        {message}
                      </div>
                    )}
                    ...
                    <div className="mt-8 flex flex-wrap gap-3 border-t border-slate-200 pt-6">
                      ...
                      {selectedPackageId && (
                        <button
                          ...
                          disabled={loading}
                          className="rounded-full border border-red-300 bg-red-50 px-6 py-3 text-sm font-semibold text-red-600 hover:bg-red-100 disabled:opacity-60"
                        >
                          Delete package
                        </button>
                      )}
                  ...
                  <div className="flex flex-wrap gap-4 items-start justify-between">
                      ...
                      <div className="flex items-center gap-3 mb-1 flex-wrap">
                        ...
                        {r.packageId && (
                          <span className="rounded-full bg-blue-100 px-2 py-0.5 text-xs font-medium text-blue-700">
                            {packages.find((p) => p.id === r.packageId)
                              ?.title ?? r.packageId}
                          </span>
                        )}
                        {!r.approved && (
                    ...
                    <div className="flex flex-col gap-2 shrink-0">
                      <div className="flex items-center justify-between gap-3 rounded-2xl border px-3 py-2 text-xs">
                        ...
                        <Toggle
                          value={!!r.approved}
                          onChange={(v) => updateReview(r.id!, { approved: v })}
                        />
                      </div>
                      <div className="flex items-center justify-between gap-3 rounded-2xl border px-3 py-2 text-xs">
                        ...
                        <Toggle
                          ...
                          onChange={(v) =>
                            updateReview(r.id!, { highlightedHome: v })
                          }
                        />
                      </div>
                      <div className="flex items-center justify-between gap-3 rounded-2xl border px-3 py-2 text-xs">
                        ...
                        <Toggle
                          value={!!r.highlightedPackage}
                          onChange={(v) =>
                            updateReview(r.id!, { highlightedPackage: v })
                          }
                        />
                      </div>
                      <button
